...
- 新增 §6.1 Operator 钱包页（`/admin/operators`，仅管理员，导航“管理”组紧跟在“审计”之后，标签 `Operator`）：
  ...
  - 操作按状态（`removed` 优先于 `registered`/`enabled` 判断；`removed` 与 `enabled` 是后端两个独立字段，可能同时为真）：`removed && enabled` 只有 `启用`（`POST .../enable`，清摘除状态与失败计数，即“恢复”，按钮文案仍是“启用”不单独造词）+ `停用`；`removed && !enabled` 只有 `启用` + `删除`（未登记时后端只接受删除，其余操作都会 409）；两种摘除态都不给 `提回 ETH`；未登记（且未摘除）只有 `删除`；已登记未启用有 `启用`、`删除`、`提回 ETH`；已启用有 `停用`、`提回 ETH`。
