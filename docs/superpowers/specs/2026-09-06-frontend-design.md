...
- `accountsChanged` 规则改为按插件是否真的切换选中账号判断，而不是“会话地址是否还在列表里”：记录 `pluginCurrent`（插件上一次选中的账号，`notePluginAccounts` 由 `useOkxAccounts` 和事件处理共同维护）；`accounts[0]` 跟 `pluginCurrent` 一样 → 插件选中没变（锁定/解锁、单纯调整授权等）→ 不动会话，哪怕这时会话地址跟 `accounts[0]` 本来就不一致；`accounts[0]` 真的变了才看是否需要自动切换（跟会话地址不同才 `runSwitchChain`）。页面刚加载、还没有 `pluginCurrent` 时按“变了”处理，等同旧行为。
- 需要签名的操作（`signAction`、登录页 `loginAs`）在发起签名前用 `listAccounts()`（不弹窗）确认 OKX 当前选中账号是否与目标地址一致；不一致就提示“请在 OKX 里切到 `<地址>` 后重试”，不发起签名/不弹窗；一致或查不到当前账号（未安装/未授权）则照常进行，交给现有错误提示兜底。
- 测试：顶部下拉的选项 = 会话 + `saved`（含过期/插件当前标注）；选有效缓存地址不弹签名；忙碌时下拉禁用；`accountsChanged` 同一 `accounts[0]` 重复事件不触发切换、选中变化才触发、首次事件按“变了”处理；`signAction`/登录页在插件当前 ≠ 目标地址时给出提示且不签名。
